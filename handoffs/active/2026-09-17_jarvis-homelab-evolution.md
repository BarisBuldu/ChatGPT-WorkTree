...
### Fixer execution mode — Manual / Semi-Auto / Full Auto (Verified)

Fixer provider dropdown'undan ayrı, persistent bir **Execution Mode** dropdown bulunacaktır. Discord bütün modlarda görünürlük/raporlama yüzeyidir; yalnız policy'nin onay gerektirdiği action'larda execution bekleme kapısıdır.

- **Manual:** Fixer root-cause ve typed planı hazırlar. Her production mutation CT104 `#approvals` üzerinden exact kullanıcı onayı bekler. Onay sonrası executor çalışır; verification geçmeden başarı raporlanmaz.
- **Semi-Auto:** Action registry'de bu moda izinli, target-scoped, precondition/proof'u tam ve güvenli rollback'i bulunan typed fix veya update Discord onayı beklemeden yürür; başlangıç ve final sonuç Discord'a raporlanır. Risk registry'nin approval-required işaretlediği, belirsiz veya yüksek etkili action `#approvals`a gider.
- **Full Auto (Verified):** Policy/registry'nin bu moda izin verdiği low/medium ve ayrıca kanıtlanmış high-risk olmayan typed fix/update'lar Discord onayı beklemeden execute edilir; zorunlu verification ve gerekirse otomatik rollback sonrası raporlanır. Bu mod unrestricted shell veya modelin serbest karar yetkisi değildir.

**Update policy:** `update` tek risk sınıfı değildir. Semi-Auto; exact/pinned version, güvenilir source, yeterli disk/resource, uyumlu dependency, uygun maintenance window, alınmış backup/snapshot veya deterministik rollback ve gerçek post-update health kontratı bulunan patch/minor update profillerini otomatik çalıştırabilir. Full Auto registry'de ayrıca izinli daha geniş fakat rollback-capable update profillerini çalıştırabilir. Major version, schema/data migration, kernel/host/Proxmox, firmware, HA Core veya kritik add-on, network/DNS/firewall topology, storage/filesystem/mount, LXC lifecycle/resource, credential/secret ve irreversible/belirsiz blast-radius update/action varsayılan approval-required kalır. Bir target için güvenli olduğu production/sandbox kanıtıyla gösterilmeden genel `apt upgrade`, wildcard package update veya “update all” otomasyona açılmaz.

**Execution sonrası verification zorunlu başarı kapısıdır:** Action tamamlandı demek exit code 0 değildir. Executor her action profile'ında en az (1) process/systemd/container state, (2) gerçek endpoint/API/functional query, (3) kritik dependency/upstream-downstream, (4) metric/log error regression, (5) mümkünse kullanıcıya görünen iş akışı ve (6) belirlenmiş kısa soak/recheck penceresini doğrular. Expected version/config ve before/after state eşleşir. Herhangi biri başarısızsa action success sayılmaz; incident açık kalır, güvenliyse otomatik rollback çalışır, rollback ayrıca aynı katmanlarla doğrulanır ve Discord'a failure/rollback sonucu gider. Rollback mümkün değilse execution öncesinde otomatik moda uygun değildir.

**Her modda değişmeyen hard boundary:** deletion/data loss, onaysız destructive queue/blocklist/database/storage işlemi, irreversible action veya target/impact/rollback belirsizliği exact kullanıcı onayı ister. `delete_file`, `remove_queue_item`, `block_release` ve eşdeğer destructive action ajan tool allowlist'ine doğrudan girmez. Policy/audit/target/proof/verification/rollback kaynağı eksikse fail-closed durur.

Execution Mode varsayılanı **Manual**'dır. Semi-Auto veya Full Auto'ya geçiş kullanıcı kimliği, previous/new mode, actor, timestamp, expiry ve policy version ile audit edilir; Full Auto seçimi ayrı confirmation gerektirir. Mode süreli açılabilir ve Manual'a döner. Global emergency stop yeni dispatch'i keser; in-flight action reconciliation'a alınır.

Guardian actionable sorun bulduğunda stable incident/event/correlation kimliğiyle canonical kayıt oluşturur, seçili Fixer'a structured handoff verir ve CT104 Discord incident/thread bildirimi gönderir. Manual'da plan+approval; Semi/Full'da otomasyona uygun action için Discord teslimi beklenmeden execution başlayabilir, fakat `started`, verification, rollback ve final result durable biçimde aynı thread'e teslim edilir. CT104 down iken approval-required action fail-closed bekler; auto-eligible action'ın Discord delivery'si durable spool'a alınır ve policy ayrıca `discord_required_before_auto` diyorsa o action da bekler. Replay duplicate execution üretmez.

**Incident-specific risk decision:** Otomasyon kararı sabit örnek listelerinden veya “bütün update'leri yap” kuralından türetilmez. Guardian/Fixer her incident için canlı evidence üzerinden blast radius, target criticality, dependency count, data-loss ihtimali, reversibility, rollback confidence/time, backup freshness, state drift, change scope/version delta, security exposure, current load/maintenance window, detection confidence ve previous failure history boyutlarını yapılandırılmış risk assessment olarak üretir. Model önerilen risk/eylem gerekçesini açıklar; ayrı deterministic policy engine hard stopper'ları ve minimum risk floor'u uygular, eksik/çelişkili evidence'da riski yükseltir veya approval ister. Model risk sınıfını düşürerek policy'yi aşamaz. Yeni/untyped action otomatik çalışmaz; önce bounded typed executor contract gerekir.

Semi-Auto ve Full Auto'da incident-specific değerlendirme action'ın o anda otomatik uygun olmadığını belirlerse approval request her zaman CT104 `#approvals`a gider. Semi/Full hiçbir approval-required olayı sessizce atlamaz veya yalnız logda bırakmaz. Aynı action farklı target/state/backup/dependency koşullarında farklı risk ve approval sonucu alabilir; kararın input'ları audit edilir.

**Semi/Full approval continuation:** Semi-Auto ve Full Auto approval-required incident yüzünden genel olarak durmaz veya capability'yi reddetmez. Yalnız o incident `AWAITING_APPROVAL` durumuna geçer; diğer uygun işler devam eder. Kullanıcı Discord'da onay verdiğinde aynı `incident_id`/plan revision/digest ile typed executor otomatik devam eder, action'ı uygular, zorunlu verification/soak yapar, gerekirse rollback eder ve sonucu aynı thread'e teslim eder. Onay ret/timeout olursa yalnız ilgili incident bekler/kapanmaz. Ancak untyped action, eksik target/proof, doğrulanamayan impact, olmayan rollback veya hard policy deny onayla dahi yürütülmez ve açık `BLOCKED` gerekçesi verir.

Mode kararı ayrı policy/executor boundary'dedir; model risk sınıfını düşüremez. Action registry `allowed_modes`, target selector, risk, preconditions, approval rule, version constraints, backup/rollback, verification contract, soak window, maximum frequency, cooldown ve circuit breaker taşır. Aynı target lock, state-drift recheck ve idempotency zorunludur.

Command Center Guardian provider, Fixer provider, Execution Mode, pause/emergency stop, policy version, otomasyona izinli fix/update profilleri, son action, verification/soak durumu, bekleyen approval, rollback ve Discord delivery durumunu gösterir. Unknown/stale policy veya verifier yeşil/full-auto-ready gösterilmez.
...
- Butonlar en az `Bir Kez Onayla`, `Reddet`, `Detay`, `Ertele` içerir.
- Approval kartı ve incident thread'i iki yönlüdür; yalnız buton/monolog değildir. Configured owner thread'e doğal dille `önce backup al`, `yalnız CT122'yi düzelt`, `bu geceye ertele`, `restart yapma, config'i geri al`, `daha fazla kanıt getir` veya alternatif bir hedef/constraint yazabilir.
- Kullanıcı mesajı raw shell veya doğrudan execution emri değildir. CT104 kimliği ve incident/revision bağını doğrular; Fixer mesajı structured constraint/plan-change request olarak yorumlar, gerekiyorsa soru sorar ve revised typed plan + yeni impact/verification/rollback üretir.
- Target, action, impact, zaman, scope veya rollback'i değiştiren mesaj mevcut approval digest'ini geçersiz kılar; yeni revision ve yeni onay gerekir. Salt açıklama isteği veya kanıt talebi execution yetkisi yaratmaz.
- Fixer kullanıcıya aynı thread içinde gerekçeli cevap verir; belirsiz talepte tahminle execution yapmaz, clarification ister. Conversation transcript canonical audit'e actor/timestamp/revision ile eklenir; secret/redaction kuralları sürer. Onay yalnız configured owner Discord user ID/role için geçerlidir; başka kullanıcı, expired nonce, replay, değiştirilmiş plan/target veya state drift reddedilir.
...
**2026-09-22 son P7 uygulayıcı ve remote sonucu:** Jarvis `abf76635ccf6bbe95cc6ffdef2b235973d25a9f8` ve webapp `5db473dcd9522e743d3036781b2a68a43c3d0baf` commitleri raporlandı; Cloud GitHub kontrolünde `BarisBuldu/jarvis` `main` ve `BarisBuldu/webapp` `master` branch HEAD'leri bu SHA'larla birebir eşleşti. CT105 sandbox'ta restore edilmiş `pipeline.sql` ile gerçek CT122 Subtitle Pipeline `api_server.py` başlatılıp 809 gerçek processed kayıt sorgulandığı; Guardian feedback suppression'ın sonraki taramayı etkilediği, stale source'un UI'da yeşil görünmediği ve önceki durable CT104/Discord replay kanıtının canonical evidence'a bağlandığı bildirildi. P7-02 ve P7-04 PASS kaydedilir. P7-01 kullanıcı Deco'da primary DNS'i CT124 `192.168.68.75` yapıp secondary'yi boş bırakana ve gerçek istemci/failure kanıtı gelene kadar OPEN'dır. P7-03 gerçek servis start/functional sorgu göstermiştir ancak CT140 yalnız SQL dump tuttuğu, application code/venv/config restore artifact'inde bulunmadığı için restore-capable backup kapsamı tamamlanmadan OPEN kalır. P7-05 final gate bu iki madde nedeniyle OPEN; Phase 8 başlamaz.

**2026-09-22 `92d04a8` kapanış iddiası denetimi:** Jarvis `main` remote HEAD `92d04a883bc05209650d6eb82f991ffc7b0906f3` olarak bağımsız doğrulandı. Deco istemci yönlendirmesi, gerçek query logları, filtered domain ve CT124 service-stop sırasında sıfır public bypass P7-01'i PASS yapar. Yeni CT140 artifacts ve gerçek `api_server.py`/809-row sorgusu P7-03'te ilerlemedir; ancak commit'teki verifier `/status` içeriğini assert etmeden `status_ok=True` ve `pass=True` yazar, kaydedilen cevap `{"running": false, "queue": [], "queue_length": 0}`'dır. Manifestler sandbox'a kopyalansa da fresh venv/dependency reconstruction yapılmaz; yeni artifacts için off-site copy/hash kanıtı yoktur. Bu nedenle Jarvis handoff'undaki `FORMALLY CLOSED` WorkTree canonical kabulü değildir. P7-03 ve P7-05 OPEN, Phase 7 OPEN; Phase 8 başlamaz.

**2026-09-22 `9f5c0c6` teknik düzeltme ve canonical drift sonucu:** Jarvis remote `main` HEAD `9f5c0c6ee085075d3118ec825c5bdbbbef6f8b36` bağımsız doğrulandı. Fresh venv/offline wheelhouse, dinamik health AND koşulu, tekrarlı probes, 809-row functional query, dört negatif FAIL testi, daily state ve Google Drive 0-difference/6-match kanıtı önceki P7-03 teknik kusurlarını giderir. Fakat CT140 üzerinde değiştirilmiş `/opt/backup.sh` repository tree'sinde versioned değildir; commit dosya listesi yalnız verifier/test/evidence/state belgelerini içerir. Evolution'ın clone-only canonical recovery kuralı gereği bu production artifact generator'ın secretsiz deployable kaynağı repo'ya eklenmeli, kurulum yolu ve production script hash eşleşmesi kanıtlanmalıdır. Bu canonical drift tek kalan P7-03 blocker'ıdır; kapanmadan P7-05/Phase 7 CLOSED değildir ve Phase 8 başlamaz.

**2026-09-22 `ccf208a` canonical script ve credential denetimi:** Jarvis remote `main` HEAD `ccf208ad21b2354366c04657a993bd3ebceca993` bağımsız doğrulandı. Versioned generator ve verifier committe gerçek olarak `scripts/ct140-backup.sh` ile `scripts/verify_ct140_backup_script.py` yollarındadır; template SHA256 rapor edilen `3d1e45c2e80d8a656f219bb19d55ffbf1321b935ce6cb2021649b225fa196972` ile eşleşir. GitHub'a backup verisi değil otomasyon kodu alınmıştır. Buna karşın verifier gerçek Firefly DB parolasını source literal olarak içerip aynı değeri runtime render için kullanır; runtime `/opt/backup.sh` da secret gömülü halde tutulmaktadır. Bu §30 secret-handling ve clone-safe canonical state kurallarını ihlal eder. Runtime secret protected env/credential dosyasına taşınmalı, runtime ve repo scripti secretsiz/direct byte eşleşmeli, verifier hiçbir gerçek secretı source/diff/loga almadan doğrulamalı ve Git history exposure kullanıcı onayıyla ele alınmalıdır. Yeni production + commit/push/remote kanıt gelmeden P7-03/P7-05 ve Phase 7 OPEN/BLOCKED; Phase 8 başlamaz.
