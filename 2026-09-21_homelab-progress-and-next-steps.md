...
- [ ] **P7-01 — CT124 DNS adblock:** AdGuard Home kurulumu/engelleme uygulayıcı tarafından raporlandı; Deco istemcileri hâlâ CT124'ü kullanmıyor. Gerçek istemci sorgusu, bypass etmeyen failover/privacy, rollback ve kullanıcı görünümü kanıtı olmadan açık kalır. Router değişikliği kullanıcı onayı/eylemi bekler.
...
- [ ] **P7-02 — storage/media/network/HA:** Dinamik medya sayıları, scanner kartı ve operatör butonları raporlandı. Ancak feedback'in sonraki taramaya etkisi, HA update içeriği/gerekçeli triage, bütün network/history kapsamı ve failure/stale kullanıcı davranışı tek kanıt zincirinde gösterilmedi. Gerçek tahmin/cleanup adayları ile stuck/failed/missing/quality medya kaynak/tazelikle doğrulanır.
- [ ] **P7-03 — CT140 restore:** Düzenli timer ile L1/L2 ve CT105'te restore edilmiş DB'yi okuyan test mikroservisi raporlandı. Bu, artifact'ten gerçek kritik servisin bağımlılıklarıyla **start + health** kanıtı değildir; uygun periyodik gerçek servis restore testi geçmeden açık kalır. Günlük CT140 yedeği korunur.
- [ ] **P7-04 — Guardian/Discord:** Uygulayıcı kontrollü fixture için disk outbox → pending → CT104/Discord delivery → restart replay/dedup zincirini ve message/thread kimliklerini raporladı. Exact commit, push/remote doğrulama ve Cloud-bağımsız runtime kontrolü yok; ayrıca süreli feedback'in sonraki scanner kararına etkisi açıkça gösterilmedi. Bu nedenle canonical completion henüz verilmez.
